            ...
            <div class="row">
                ...
                <div class="col_10">
                    ...
                    if (isset($_POST["submit"]))
                    {
                        ...
                        echo "<h2>Project Number: <em>" . $strProjectNumber . "</em></h2><br/><br/>";  //want this displayed above an eventual error msg
                        
                        validateProject(editproject);  //function is in validate_data.php file
        
                        if ($booProjectOk)  //$booProjectOk is updated in validateProject()
                        {                            
                            //Redirect to the Projects page
                            header("Location: projects.php?SUCCESS_MESSAGE=editDone");
                        }					
                    }
                    else  //else this is true: isset($_GET[PROJECT_NUMBER]) 
                    {
                        //Collect PROJECT_NUMBER which is assigned Project Number variable in url on projects.php page PROJECT_NUMBER=$arrRows[Project_No]
                        $strProjectNumber = $_GET[PROJECT_NUMBER];
                        
                        echo "<h2>Project Number: <em>" . $strProjectNumber . "</em></h2><br/><br/>";               
                        
						//get project
						readQuery("d_project", array("Project_No"), array($strProjectNumber), array(true));  //in db_functions.php
        
                        // if there is a record continue
                        if ($numRecords == 0) 
                        {
                            echo "<span class='error'>No Matching Project Found!</span><br /><br />";
                        } 
                        else  //should be only one
                        {
                            ...
                            $strProjectNumber = $arrRows['Project_No']; //redundant because it is already set
                            $strProjectName = $arrRows['Project_Name'];  
                            $strProjectDescription = $arrRows['Project_Description']; 
                            $strProjectManager = $arrRows['Project_Manager']; 
                            $strProjectStartDate = $arrRows['Start_Date']; 
                            $strProjectFinishDate = $arrRows['Finish_Date']; 
                            $strProjectBudget = $arrRows['Budget']; 
                            $strProjectCostToDate = $arrRows['Cost_To_Date']; 
                            $strProjectTrackingStatement = $arrRows['Tracking_Statement']; 
                            $strProjectClientNumber = $arrRows['Client_No'];			
                        }					
                    }
                    ...
                    echo "<tr><th>Project name *</th><td><input type='text' name='strProjectName' maxlength='50' size='38' value='".htmlspecialchars($strProjectName, ENT_QUOTES)."' /></td>";
                    if ($booProjectName)
                    ...
                    //build select menu for Consultant ID				
                    ...
                    //build select menu for Client Number			
                    while($arrRows = $stmt->fetch(PDO::FETCH_ASSOC))
                    {							
                        //decide which to be the selected
                    ...
                    echo "<input class='button' type='submit' name='submit' value='Submit Edited Details' />";
                    echo "<input type='hidden' name='strProjectNumber' value='".$strProjectNumber."' /></form>";
