        ...
        <section>
            ...
            <div class="row">
                ...
                <div class="col_10">
                    ...
                    {
                        //redirect to the projects.php page
                        header("Location: projects.php");	
                    }	
					else if (isset($_POST["okDelete"])) //only proceed to delete if "Yes Delete" button is pressed
                    {
						echo "<h2>Project Name: <em>" . $_POST[PROJECT_NAME] . "</em></h2><br/>";
						echo "<h2>Project Number: <em>" . $_POST[PROJECT_NUMBER] . "</em></h2><br/><br/>";
						
						echo "<form action='delete_project.php' method='post'>";
						
						echo '<div class="row marginTopBottom1point5em">';
						...
						echo '<input type="hidden" name="PROJECT_NUMBER" value=' . $_POST[PROJECT_NUMBER] . '>';
						echo '<input type="hidden" name="PROJECT_NAME" value="' . $_POST[PROJECT_NAME] . '"></div>';
						echo '</div>';							
									
						echo "</form>";		
														
						//make arrays			
						$whereColumnNamesArray = array("Project_No");  //arr holds the column names 
						$whereColumnValuesArray = array($_POST[PROJECT_NUMBER]);	//get project number from hidden input						
                        ...
                        $booDeleteDone = deleteRecord("d_project", $whereColumnNamesArray, $whereColumnValuesArray, $whereColumnValuesQuotesArray);  
                        
                        //check if the record has been deleted
                        if ($booDeleteDone)
                        {
                            //redirect to the projects page if no errors occurred in deleteRecord()
                            header("Location: projects.php?SUCCESS_MESSAGE=deletedDone");
                        }
							
					}
					else //or it comes from the projects page
					{
						echo "<h2>Project Name: <em>" . $_GET[PROJECT_NAME] . "</em></h2><br/>";
						echo "<h2>Project Number: <em>" . $_GET[PROJECT_NUMBER] . "</em></h2><br/><br/>";
						...
						echo '<div class="col_2"><input class="button" type="submit" name="okDelete" value="Yes Delete"></div>';
						echo '<div class="col_10"><input class="button" type="submit" name="cancel" value="Cancel">';
						echo '<input type="hidden" name="PROJECT_NUMBER" value=' . $_GET[PROJECT_NUMBER] . '>';
						echo '<input type="hidden" name="PROJECT_NAME" value="' . $_GET[PROJECT_NAME] . '"></div>';
						echo '</div>';							
